...
const JoinedRidesPage: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const myJoined = await ridesApi.getJoinedRides();
      setRides(myJoined);

      const counts: Record<number, number> = {};
      await Promise.all(
        myJoined.map(async (ride) => {
          try {
    ...
    >
      ...
      {rides.length > 0 ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
          <p className="text-gray-500 mb-4">You haven't joined any upcoming trips.</p>
          <Button onClick={() => navigate('/join')}>Find a Ride</Button>
        </div>
      )}
    </motion.div>
  );
...
export default JoinedRidesPage;
